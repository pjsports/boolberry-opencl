...
#define OUTPUT_SIZE 16
	...
	bool simpleminer::run()
	{
		...
		while(true)
		{
			...
			if (m_threads_total > 1) { 
				LOG_PRINT_RED_L0("OpenCL simpleminer should be used 1 instance per GPU! Changing to 1 mining thread.");
				m_threads_total = 1;
			}
	...
	void simpleminer::update_fast_scratchpad()
	{
		...
		if (cur_scratchpad_pages > m_fast_scratchpad_pages)
		{
			...
			m_fast_scratchpad_pages = (uint32_t)cur_scratchpad_pages;
