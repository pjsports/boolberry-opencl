...
//	log_space::log_singletone::add_logger(LOGGER_FILE, log_space::log_singletone::get_default_log_file().c_str(), log_space::log_singletone::get_default_log_folder().c_str());
...
//			nonce_offset += ((m_threads_total-1) * attempts_per_loop);
		...
		while(true)
		{
			...
			if(!m_http_client.is_connected())
			{
				...
				if (re_get_scratchpad || !m_hi.height || !m_scratchpad.size())
				{
					...
					search_start = epee::misc_utils::get_tick_count();
				}
			...
			while(!done && epee::misc_utils::get_tick_count() - m_last_job_ticks < 3000)
			{
	...
	void simpleminer::update_fast_scratchpad()
	{
		...
		uint32_t cur_scratchpad_pages = (uint32_t)(cur_scratchpad_size / 4096) + 1;
		if (cur_scratchpad_pages > m_fast_scratchpad_pages)
		{
			free_fast_scratchpad();
			size_t mapsize = cur_scratchpad_pages * 4096;
			void *addr = malloc(mapsize);
